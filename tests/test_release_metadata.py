...
def test_release_version_metadata_is_consistent():
    ...
    assert gamestick.__version__ == "0.5.0-alpha13.1"
    assert 'version = "0.5.0a13.post1"' in pyproject
    assert 'VERSION = "0.5.0-alpha13.1"' in ui
    assert readme.startswith("# GameStick Inspector 0.5.0-alpha13.1")
...
def test_windows_launchers_bootstrap_fresh_extract():
    ...
    for launcher in ("run_admin.bat", "probe.bat", "image.bat", "compare.bat", "fast_compare.bat", "catalogue_compare.bat", "repair_workspace.bat", "rom_hide.bat", "custom_apply.bat", "test.bat"):
